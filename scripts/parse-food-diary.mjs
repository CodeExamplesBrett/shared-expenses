/**
 * One-time parser: Obsidian "Food Diary" vault folder -> src/assets/food-diary-seed.json
 *
 * Parses the English section of each YYYY-MM-DD.md entry (meal tables with
 * ingredient/amount/unit price/cost) plus "_Ingredient Prices.md", and writes a
 * seed file the app can import into Firestore.
 *
...
function parseMealBlock(lines, context) {
...
function parseDay(file) {
  ...
  for (const section of sections) {
    ...
    const meal = parseMealBlock(lines.slice(1), `${date}/${key}`);
...
function parsePrices() {
  const raw = readFileSync(join(SRC_DIR, '_Ingredient Prices.md'), 'utf8');
  const prices = [];
...
const dayFiles = readdirSync(SRC_DIR)
  .filter((f) => /^\d{4}-\d{2}-\d{2}\.md$/.test(f))
  .sort();

const days = dayFiles.map(parseDay).filter(Boolean);
...
console.log(
  days.map((d) => `  ${d.date}  €${d.totalCost.toFixed(2)}`).join('\n'),
);
if (warnings.length) {
  console.log('\nWarnings:');
